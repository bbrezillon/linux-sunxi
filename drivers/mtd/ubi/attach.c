...
static void ubi_scan_peb_reset(struct ubi_scan_peb *speb, int pnum)
{
	speb->bad = false;
	speb->ec_scanned = false;
	speb->vid_scanned = false;
	speb->ec_err = 0;
	speb->vid_err = 0;
}

static int ubi_scan_peb_init(struct ubi_device *ubi,
			      struct ubi_scan_peb *speb,
			      struct ubi_ec_hdr *ech,
			      struct ubi_vid_io_buf *vidb,
			      int pnum)
{
...
static int ubi_scan_peb_ec(struct ubi_device *ubi,
			   struct ubi_scan_peb *speb)
{
	speb->ec_err = ubi_io_read_ec_hdr(ubi, speb->pnum, ech, 0);
	if (speb->ec_err < 0)
		return speb->ec_err;

	speb->ec_scanned = true;

	return 0;
}

static int ubi_scan_peb_vid(struct ubi_device *ubi,
			    struct ubi_scan_peb *speb)
{
	speb->vid_err = ubi_io_read_vid_hdr(ubi, speb->pnum, speb->vidb, 0);
	if (speb->vid_err < 0)
		return speb->vid_err;
...
static int ubi_scan_peb(struct ubi_device *ubi, int pnum,
			struct ubi_scan_peb *speb)
{
	struct ubi_ec_hdr *ech = speb->ech;
	struct ubi_vid_io_buf *vidb = speb->vidb;
	struct ubi_vid_hdr *vidh = ubi_get_vid_hdr(vidb);
	long long ec;
	...
	speb->ec_err = ubi_io_read_ec_hdr(ubi, pnum, ech, 0);
	if (speb->ec_err < 0)
		return speb->ec_err;

	speb->ec_scanned = true;

	if (speb->ec_err == UBI_IO_FF || speb->ec_err == UBI_IO_FF_BITFLIPS)
		return 0;
