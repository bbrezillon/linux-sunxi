 ...
 * @ubi: UBI device description object
 * @pdesc: PEB descriptor
 * @torture: if the physical eraseblock has to be tortured
 * @nested: true if schedule_erase() is called with ->work_sem held
 *
 * This function returns zero in case of success and a %-ENOMEM in case of
 * failure.
 */
static int schedule_erase(struct ubi_device *ubi,
			  ...
			  bool nested)
{
	...
	e = ubi->lookuptbl[pdesc->pnum];

	dbg_wl("schedule erasure of PEB %d, EC %d, torture %d",
	       pdesc->pnum, e->ec, torture);
 ...
 *
 */
static int do_sync_erase(struct ubi_device *ubi,
			 const struct ubi_peb_desc *pdesc, int torture)
{
	struct ubi_erase_work wrk;
	struct ubi_wl_entry *e = ubi->lookuptbl[pdesc->pnum];
	int ret;
					    ...
					    struct ubi_vid_io_buf *vidb)
{
	...
	ubi_assert(nhdrs <= ubi->max_lebs_per_peb);
	...
	pdesc->pnum = pnum;
	pdesc->vol_id = be32_to_cpu(vidh->vol_id);

	for (i = 0; i < nhdrs; i++) {
		pdesc->lnums[i] = be32_to_cpu(vidh[i].lnum);
		ubi_assert(be32_to_cpu(vidh[i].vol_id) == pdesc->vol_id);
	}
				...
				int shutdown)
{
	...
	if (scrubbing) {
		...
		for (i = 0; i < ubi_get_nhdrs(vidb); i++) {
			int lnum = be32_to_cpu(vid_hdr[i].lnum);

			if (lnum >= 0)
				ubi_msg(ubi,"\tLEB %d:%d", vol_id,
					be32_to_cpu(vid_hdr[i].lnum));
		}
	}
	...
	pdesc = pdesc_from_vidb(ubi, e1->pnum, vidb);
	if (!pdesc) {
		err = -ENOMEM;
		goto out_ro;
	}

	err = do_sync_erase(ubi, pdesc, 0);
	if (err) {
		if (e2)
	...
	if (e2) {
		 ...
		 * erasure.
		 */
		pdesc = pdesc_from_vidb(ubi, e2->pnum, vidb);
		if (!pdesc) {
			err = -ENOMEM;
			goto out_ro;
		}
		...
		for (i = 0; i < ubi->max_lebs_per_peb; i++) {
			if (pdesc->lnums[i] > 0)
				ubi_err(ubi,"\tLEB %d:%d", pdesc->vol_id,
					pdesc->lnums[i]);
		}
...
out_not_moved:
	...
	if (dst_leb_clean) {
		...
		err = do_sync_erase(ubi, pdesc, torture);
		if (err)
			goto out_ro;
	}
...
static int __erase_worker(struct ubi_device *ubi,
			  struct ubi_erase_work *wrk)
{
	const struct ubi_peb_desc *pdesc = wrk->pdesc;
	int pnum = pdesc->pnum;
	...
	for (i = 0; pdesc->vol_id >= 0 && i < ubi->max_lebs_per_peb; i++) {
		if (pdesc->lnums[i] >= 0)
			dbg_wl("\tLEB %d:%d", pdesc->vol_id, pdesc->lnums[i]);
	}
	...
	if (!err) {
		...
		/* Free the PEB desc before leaving. */
	...
	if (err == -EINTR || err == -ENOMEM || err == -EAGAIN ||
		...
		err1 = schedule_erase(ubi, pdesc, 0, false);
		if (err1) {
		...
		/*
		 * Do not free the PEB descriptor here: it has been passed to
		 * a new work.
		 */
		return err;
	}
	...
	/* Free the PEB descriptor before leaving. */
	ubi_free_pdesc(pdesc);
			  ...
			  int shutdown)
{
	...
	if (shutdown) {
		struct ubi_wl_entry *e = ubi->lookuptbl[ewrk->pdesc->pnum];

		dbg_wl("cancel erasure of PEB %d EC %d", e->pnum, e->ec);
		ubi_free_pdesc(ewrk->pdesc);
		kfree(ewrk);
		wl_entry_destroy(ubi, e);
		return 0;
	}
 ...
 *
 * In case of error, this function takes care of releasing the @pdesc resource.
...
retry:
	...
	if (e == ubi->move_from) {
		 ...
		 */
		dbg_wl("PEB %d is being moved, wait", pdesc->pnum);
		mutex_unlock(&ubi->wl_lock);
		 ...
		 */
		dbg_wl("PEB %d is the target of data moving", pdesc->pnum);
		ubi_assert(!ubi->move_to_put);
		ubi->move_to_put = 1;
		mutex_unlock(&ubi->wl_lock);
		...
		if (in_wl_tree(e, &ubi->used)) {
			...
			if (err) {
				ubi_err(ubi, "PEB %d not found", pdesc->pnum);
				ubi_ro_mode(ubi);
				mutex_unlock(&ubi->wl_lock);
				up_read(&ubi->fm_protect);
				ubi_free_pdesc(pdesc);
	...
	err = schedule_erase(ubi, pdesc, torture, false);
	if (err) {
		mutex_lock(&ubi->wl_lock);
		wl_tree_add(e, &ubi->used);
		mutex_unlock(&ubi->wl_lock);
 ...
 */
static bool ubi_work_match(struct ubi_device *ubi, struct ubi_work *wrk,
			   int vol_id, int lnum)
{
...
int ubi_wl_flush(struct ubi_device *ubi, int vol_id, int lnum)
{
	...
	while (found) {
		...
		list_for_each_entry_safe(wrk, tmp, &ubi->works, list) {
			if (ubi_work_match(ubi, wrk, vol_id, lnum)) {
				list_del(&wrk->list);
				ubi->works_count -= 1;
				ubi_assert(ubi->works_count >= 0);
				mutex_unlock(&ubi->wl_lock);
...
int ubi_wl_init(struct ubi_device *ubi, struct ubi_attach_info *ai)
{
	...
	list_for_each_entry_safe(apeb, tmp, &ai->erase, node) {
		...
		if (schedule_erase(ubi, pdesc, 0, false)) {
			ubi_free_pdesc(pdesc);
			wl_entry_destroy(ubi, e);
			goto out_free;
		}
